...
当前候选版本为 `v1.5.0-rc.1`。这是在线测试版本，尚未迁移到 `main`。
...
输出位于 `android/app/build/outputs/apk/debug/app-debug.apk`。当前 Android 元数据为 `versionCode 16`、`versionName 1.5.0-rc.1`。

## 云端部署边界

- API 镜像只使用提交 SHA 标签部署，不依赖可变的版本标签。
- 数据库迁移在部署前显式执行，不在容器启动时隐式修改数据库。
- 部署前必须创建并验证 PostgreSQL 备份。
- 云同步采用乐观版本号；发生冲突时暂停自动覆盖，由用户选择“本机覆盖云端”或“从云端恢复”。
