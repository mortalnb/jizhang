...
const findCategory = (text: string, categories: string[]) => {
  ...
  if (/饭|餐|咖啡|奶茶|面包|水果|啤酒|外卖|火锅|麦当劳|盒马/.test(text)) return categories.includes('餐饮') ? '餐饮' : categories[0];
      ...
      const response = await fetch(`${settings.baseUrl}/v1/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: `你是记账助手。请从用户输入中提取 JSON：amount, category, paymentMethod, description, date, tag, splitItems。category 必须属于：${categories.join(', ')}。今天是 ${todayISO()}。只返回 JSON。`,
            },
